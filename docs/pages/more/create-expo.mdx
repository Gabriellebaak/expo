...
const App = () => {
  const [relaxationTechnique, setRelaxationTechnique] = useState('');

  const startRelaxation = () => {
    // Logic to start relaxation technique
    setRelaxationTechnique('Respiration profonde');
  };

  return (
    <View style={styles.container}>
      <Text style={styles.title}>SleepWell</Text>
      <Button title="Commencer la relaxation" onPress={startRelaxation} />
      {relaxationTechnique !== '' && (
        <Text style={styles.technique}>{relaxationTechnique}</Text>
      )}
    </View>
  );
};
